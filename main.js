...
chatcomponent.innerHTML += ` 
       <div> 
         ...
         <svg class="bubble" viewBox="0 0 77 75" fill="none" >
                <ellipse cx="38.5" cy="37.5" rx="38.5" ry="37.5" fill="#8c0fca"/>
...
var xtime= new Date()
var hour=xtime.getHours();
var minute=xtime.getMinutes();
var second=xtime.getSeconds();
if(hour <10 ){hour='0'+hour;}
if(minute <10 ) {minute='0' + minute; }
var xtime3 ="Today at "+ ' ' +  hour+':'+minute+':'+second
...
function getMessage(obj) {
  ...
  message.className = "messageoOperator"
  if (obj.answer) {
...
`;
      document.querySelector(".typemessa").remove()
...
const handleSubmit = async (e) => {
  ...
  if (data) {
    ...
    let message = document.createElement("div");
    let messaIscome = document.createElement("div");
    let value = {key:userConfig.key, user: uint32.toString(16), message: data, respond:'' };
    console.log('value', value)
    message.className = "messageUser"
    message.innerHTML = `
             
                <div class="msg">${value.message}</div>
            
            `;
    messaIscome.className = "typemessa"
    messaIscome.innerHTML = `
